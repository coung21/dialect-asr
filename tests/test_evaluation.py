...
from dialect_asr.evaluation import MultitaskMetrics
...
def test_multitask_metrics_compute_dialect_quality_and_component_losses() -> None:
    processor = FakeProcessor(
        decoded_batches=[
            ["a", "b", "c"],
            ["a", "b", "c"],
    ...
    # Region logits: [N=3, R=3], predicted IDs [0, 2, 2].
    ctc_losses = np.array([1.0, 1.2, 1.4])  # [N=3].
    dialect_losses = np.array([0.2, 0.4, 0.6])  # [N=3].
    ctc_labels = np.array([[1, -100], [2, -100], [3, -100]])
    # CTC labels: [N=3, T_text=2].
    region_labels = np.array([0, 1, 2])  # [N=3].
